...
const HistorialTransacciones = () => {
    ...
    const [openId, setOpenId] = useState(null); // Para el acordeón
        ...
        <div className="p-6">
            ...
            <div className="overflow-x-auto bg-white shadow-sm rounded-xl border border-gray-200">
                <table className="min-w-full text-sm text-left">
                    ...
                    <tbody>
                        {loading ? (<tr><td colSpan="6" className="text-center py-4">Cargando...</td></tr>) : transacciones.length === 0 ? (<tr><td colSpan="6" className="text-center py-4">No se encontraron registros</td></tr>) : (
                            transacciones.map((t) => (
                                <React.Fragment key={t.id}>
                                    <tr className="border-b hover:bg-gray-50 cursor-pointer" onClick={() => toggleDetails(t.id)}>
                                        <td className="px-2 py-2 text-center">{openId === t.id ? <ChevronUp size={16}/> : <ChevronDown size={16}/>}</td>
                                        ...
                                        <td className="px-4 py-2"><button onClick={(e) => { e.stopPropagation(); imprimirVoucher(t.id); }} className="text-blue-600 hover:text-blue-800 flex items-center gap-1 border border-blue-600 px-2 py-1 rounded text-xs"><Printer size={14}/> Voucher</button></td>
                                    </tr>
                                    {openId === t.id && (
                                        <tr className="bg-gray-50 border-b">
                                            <td colSpan="6" className="p-4">
                                                <div className="font-bold mb-2">Detalles de la transacción:</div>
